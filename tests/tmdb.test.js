...
import { fetchGenreMap, fetchMovieDetail, looksLikeFranchise, normalizeMovies, searchBarbieMovies } from "../js/tmdb.js";

// Campos que fazem um resultado da busca por texto parecer da franquia.
const FR = { genre_ids: [16, 10751], original_language: "en", vote_count: 100 };

// Por padrao neutraliza os endpoints de collection e da Mattel (resultado
// vazio), pra nao afetar os testes que so' se importam com a busca por texto.
function fakeFetch(responsesByPage) {
  return async (url) => {
    if (url.includes("/search/collection") || url.includes("/discover/")) {
...
test("searchBarbieMovies fetches every page reported by the API", async () => {
  const fetchImpl = fakeFetch({
    1: {
      results: [{ id: 1, title: "Barbie Filme 1", release_date: "2001-01-01", ...FR }],
      total_pages: 2,
    },
    2: {
      results: [{ id: 2, title: "Barbie Filme 2", release_date: "2002-01-01", ...FR }],
      total_pages: 2,
    },
  });
...
test("searchBarbieMovies stops after the safety cap even if the API reports more pages", async () => {
  ...
  for (let page = 1; page <= 10; page += 1) {
    responsesByPage[page] = {
      results: [{ id: page, title: `Barbie Filme ${page}`, release_date: `20${10 + page}-01-01`, ...FR }],
      total_pages: 10,
    };
  }
  ...
  const fetchImpl = async (url) => {
    ...
    if (url.includes("/collection/99")) {
      return {
        ...
        status: 200,
        json: async () => ({
          parts: [{ id: 2, title: "Mariposa", release_date: "2007-01-01", ...FR }],
        }),
      };
    }
    return {
...
test("searchBarbieMovies dedupes a movie present in both the text search and a collection", async () => {
  const fetchImpl = async (url) => {
    ...
    if (url.includes("/collection/99")) {
      return { ok: true, status: 200, json: async () => ({ parts: [{ id: 1, title: "Barbie Filme 1", release_date: "2001-01-01", ...FR }] }) };
    }
    return {
      ok: true,
      status: 200,
      json: async () => ({ results: [{ id: 1, title: "Barbie Filme 1", release_date: "2001-01-01", ...FR }], total_pages: 1 }),
    };
  };
...
test("looksLikeFranchise rejects documentaries and unrelated films with Barbie in the title", () => {
  assert.equal(looksLikeFranchise({ ...FR }), true);
  assert.equal(looksLikeFranchise({ ...FR, genre_ids: [99] }), false); // documentario
  assert.equal(looksLikeFranchise({ ...FR, genre_ids: [16, 80] }), false); // animacao de crime
  assert.equal(looksLikeFranchise({ ...FR, original_language: "es" }), false);
  assert.equal(looksLikeFranchise({ ...FR, vote_count: 3 }), false);
  assert.equal(looksLikeFranchise({ genre_ids: [18, 10770], original_language: "en", vote_count: 0 }), false);
});

test("searchBarbieMovies drops text results that are not from the franchise", async () => {
  const fetchImpl = async (url) => {
    if (url.includes("/search/collection") || url.includes("/discover/")) {
      return { ok: true, status: 200, json: async () => ({ results: [] }) };
    }
    return {
      ...
      json: async () => ({
        results: [
          { id: 1, title: "Barbie as Rapunzel", release_date: "2002-10-08", ...FR },
          { id: 2, title: "The Trial of Klaus Barbie", release_date: "1987-01-01", genre_ids: [18], original_language: "en", vote_count: 0 },
        ],
        total_pages: 1,
      }),
    };
...
test("searchBarbieMovies trusts Mattel-produced movies even without franchise genres", async () => {
  const live = { id: 346698, title: "Barbie", release_date: "2023-07-21", genre_ids: [35, 12, 14], original_language: "en", vote_count: 5 };
  const fetchImpl = async (url) => {
    if (url.includes("/search/collection")) {
      return { ok: true, status: 200, json: async () => ({ results: [] }) };
    }
    if (url.includes("/discover/")) {
      return { ok: true, status: 200, json: async () => ({ results: [live, { id: 7, title: "Hot Wheels", release_date: "2020-01-01" }], total_pages: 1 }) };
    }
    return { ok: true, status: 200, json: async () => ({ results: [live], total_pages: 1 }) };
  };
  const movies = await searchBarbieMovies("chave-fake", fetchImpl);
  assert.deepEqual(movies.map((m) => m.id), [346698]); // Hot Wheels e da Mattel mas nao tem Barbie no titulo
});

test("searchBarbieMovies drops collection parts that are not animation/family", async () => {
  const fetchImpl = async (url) => {
    if (url.includes("/discover/")) {
      return { ok: true, status: 200, json: async () => ({ results: [] }) };
    }
    if (url.includes("/search/collection")) {
      return { ok: true, status: 200, json: async () => ({ results: [{ id: 5, name: "Barbie & Kendra Collection" }] }) };
    }
    if (url.includes("/collection/5")) {
      return {
        ok: true,
        status: 200,
        json: async () => ({
          parts: [
            { id: 10, title: "Corona Zombies", release_date: "2020-01-01", genre_ids: [27, 35] },
            { id: 11, title: "Barbie & Kendra Storm Area 51", release_date: "2020-01-01", genre_ids: [35, 878] },
          ],
        }),
      };
    }
